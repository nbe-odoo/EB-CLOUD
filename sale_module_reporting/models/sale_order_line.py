...
class SaleOrderLine(models.Model):
    ...
    untaxed_amount_reserved = fields.Float(compute='_compute_untaxed_reserved2', string='Untaxed Amount Reserved', store=True)
    untaxed_amount_undelivered = fields.Float(compute='_compute_untaxed_undelivered2', string='Untaxed Undelivered', store=True)
    quantity_reserved = fields.Float(compute='_compute_qty_reserved2', string='Quantity Reserved', store=True)
    quantity_undelivered = fields.Float(compute='_compute_qty_undelivered2', string='Quantity Undelivered', store=True)

    @api.multi
    @api.depends('move_ids', 'move_ids.state', 'price_unit', 'move_ids.reserved_availability')
    def _compute_untaxed_reserved2(self):
        for so_line in self:
    ...
    def _compute_untaxed_undelivered2(self):
        for so_line in self:
            for delivery in so_line.move_ids:
                if so_line.move_ids and delivery.state not in ['cancel', 'done']:
    ...
    def _compute_qty_reserved2(self):
        for so_line in self:
            for delivery in so_line.move_ids:
                if so_line.move_ids and delivery.state not in ['cancel', 'done']:
    ...
    def _compute_qty_undelivered2(self):
        for so_line in self:
            for delivery in so_line.move_ids:
                if so_line.move_ids and delivery.state not in ['cancel', 'done']:
